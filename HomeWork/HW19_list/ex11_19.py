...
def summ_first_6_elements(array):
    if len(array) < 6:
        print("Длина массива меньше 6-ти элементов")
    else:
        summ = 0
        for i in range(0, 6):
            summ += array[i]
...
# e)
def average_arithmetical_mean_all_elements(array):
    summ = 0
    for i in array:
        summ += i
...
# ж)
def average_arithmetical_mean_from_s1_to_s2_elements(array, s1, s2):
    if not 0 <= s1 < s2 < len(array):
        print("Ошибка: убедитесь, что 0 <= s1 < s2 <= размеру списка и s2 > s1.")
    else:
        summ = 0
        for i in range(s1, s2 + 1):
            summ += array[i]
        average = summ / ((s2 + 1) - s1)
        return average
...
def main():
    ...
    average_array = average_arithmetical_mean_all_elements(array)
    print("е) среднее арифметическое всех элементов массива: ", average_array, "\nИсходный:", array)
    print("\n")

    s1 = int(input("Введите начальный индекс s1 (начиная с 0): "))
    s2 = int(input(f"Введите конечный индекс s2 (меньше {len(array)}): "))
    average_arithmetical_mean_from_s1_to_s2_array = average_arithmetical_mean_from_s1_to_s2_elements(array, s1, s2)
    print("ж) среднее арифметическое элементов массива с s1-го по s2-й: ",
          average_arithmetical_mean_from_s1_to_s2_array, "\nИсходный:", array)
    print("\n")
